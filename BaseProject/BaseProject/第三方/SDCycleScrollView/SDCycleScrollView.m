...
@property (strong, nonatomic) UILabel *pageLabelControl;
...
- (void)setShowPageView:(BOOL)showPageView {
    ...
    _showPageControl = !showPageView;
    
    _pageView.hidden = !showPageView;
...
- (PageView *)pageView {
    ...
    if (!_pageView) {
        _pageView = [[PageView alloc] initWithFrame:CGRectMake(0, self.bounds.size.height - 60, self.bounds.size.width, 30)];
...
- (void)setShowPageLabelControl:(BOOL)showPageLabelControl {

    _showPageLabelControl = showPageLabelControl;
    
    if (!_pageLabelControl) {
        _pageLabelControl = [[UILabel alloc] initWithFrame:CGRectMake(self.bounds.size.width - 80, self.bounds.size.height - 80, 40, 40)];
        _pageLabelControl.layer.cornerRadius = 20;
        _pageLabelControl.clipsToBounds = YES;
        _pageLabelControl.font = [UIFont systemFontOfSize:17];
        _pageLabelControl.textAlignment = NSTextAlignmentCenter;
        _pageLabelControl.backgroundColor = [UIColor colorWithWhite:0 alpha:0.2];
        _pageLabelControl.textColor = [UIColor whiteColor];
        [self addSubview:_pageLabelControl];
        
    }
    
    _pageLabelControl.hidden = !showPageLabelControl;
...
- (void)setupPageControl
{
    ...
    _pageLabelControl.text = [NSString stringWithFormat:@"%d/%ld", indexOnPageControl + 1, (unsigned long)self.imagePathsGroup.count];
    [self.pageView setPageCount:self.imagePathsGroup.count];
...
//    if (!_pageLabelControl && _showPageLabelControl) {
//        _pageLabelControl = [[UILabel alloc] initWithFrame:CGRectMake(self.bounds.size.width - 80, self.bounds.size.height - 80, 40, 40)];
//        _pageLabelControl.layer.cornerRadius = 20;
//        _pageLabelControl.clipsToBounds = YES;
//        _pageLabelControl.font = [UIFont systemFontOfSize:17];
...
//    }
    ...
    if (_showPageLabelControl) {
        _pageLabelControl.frame = CGRectMake(self.bounds.size.width - 70, self.bounds.size.height - 70, 40, 40);
    }
    
    if (_showPageView) {
        self.pageView.frame = CGRectMake(0, self.bounds.size.height - 60, self.bounds.size.width, 30);
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    _pageLabelControl.text = [NSString stringWithFormat:@"%d/%ld", indexOnPageControl + 1, (unsigned long)self.imagePathsGroup.count];
    self.pageView.currentPage = indexOnPageControl;
}
