    ...
    if (parameter == nil) {
        ...
        NSURLSessionDataTask *task = [session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
            ...
            if (!error){
                ...
                //如果后端的方法名是GetChinaCitys，那么不需要解码  || [method isEqualToString:@"GetNews"]   || [method isEqualToString:@"GetAreaLastDate"]
                if ([method isEqualToString:@"GetImageCarousel--------"] || [method isEqualToString:@"GetAreaList"]) {//|| [method isEqualToString:@"GetHomeReProduct"]
        ...
        NSURLSessionDataTask *task = [session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
            ...
            if (!error){
                ...
                if (strlen(cString) > length){
                    ...
                    if ([method isEqualToString:@"GetSymbolsKline"]) {
                        NSLog(@"K线数据获取成功，%ld",decryptStr.length);
                    }else{
                        NSLog(@"后台返回，解码后的数据：\n%@",decryptStr);
                    }
...
+ (void)AFHTTPSessionManager:(NSString *)method parameter:(NSDictionary *)parameter success:(void (^)(id responseObject))successs failure:(void(^)(NSError *error))failure{
    ...
    //成功的，原生请求方式
    NSString *urlString = [NSString stringWithFormat:@"%@%@.do",Interface_URL ,method];
